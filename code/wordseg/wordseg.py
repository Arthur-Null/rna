...
def indexOfSortedSuffix(doc, max_word_len, rna_len):
    """
    ...
    indexes = []
    length = len(doc)
    num = int((length) / (rna_len + 1))
    #print(str(length) + '\t' + str(rna_len))
    #print(str(num))
    for k in range(num):
        shift = k * (rna_len + 1)
        for i in range(0, rna_len):
            for j in range(i + 1, min(i + 1 + max_word_len, rna_len + 1)):
                indexes.append((i + shift, j + shift))
...
class WordSegment(object):
    ...
    """
    Main class for word segmentation
    ...
    def __init__(self, doc, max_word_len=5, min_freq=0.00005, min_entropy=2.0, min_aggregation=50):
        ...
        #print(int(len(self.word_with_freq)))
        sorted(self.word_with_freq, key=lambda w:w[1], reverse=True)
        self.word_with_freq = self.word_with_freq[0:self.N]
        self.words = [w[0] for w in self.word_with_freq]
        #print(int(len(self.words)))
        #if '-' in self.words:
            #self.words.remove('-')
    ...
    def genWords(self, doc):
        ...
        """
        suffix_indexes = indexOfSortedSuffix(doc, self.max_word_len, self.rna_len)
        word_cands = {}
        ...
        length = len(doc)
        num = int((length) / (self.rna_len + 1))
        for k in word_cands:
            word_cands[k].compute(length - num)
    ...
    def segSentence(self, sentence, method=L):
        """
...
if __name__ == '__main__':
    doc = 'AAATTTCCCTGAGGGGCTAAGAGGCCC'
    ws = WordSegment(doc, max_word_len=3, min_aggregation=1.2, min_entropy=0.4)
