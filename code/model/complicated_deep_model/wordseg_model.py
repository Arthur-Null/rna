...
from code.feature_engineering import get_data_wordseg
...
data, label = get_data_wordseg(data_path="../../dataset/trainset_wordseg/trainset/")
...
class Simple_Deep:
    ...
    def _build_graph(self):
        ...
        print(batchsize,self.input_s)
        x = tf.reshape(self.input, [batchsize, self.para['len'], 4])
        x_s = tf.reshape(self.input_s, [batchsize, 200, 1])
        conv_s = tf.layers.conv1d(x_s, 16, kernel_size=2, activation=tf.nn.relu)
        conv = tf.layers.conv1d(x, 16, kernel_size=4, activation=tf.nn.relu)
        out = tf.layers.max_pooling1d(conv, 3, strides=3)
        out = tf.nn.dropout(out, self.keep_prob)
        out_s = tf.layers.max_pooling1d(conv_s, 2, strides=2)
...
if __name__ == '__main__':
    protein_list = ['AGO1', 'AGO2', 'AGO3', 'ALKBH5', 'AUF1', 'C17ORF85', 'C22ORF28', 'CAPRIN1', 'DGCR8', 'EIF4A3',
                    ...
                    'U2AF65',
                    'WTAP', 'ZC3H7B']
    para = {'len': 300, 'label_dim': 1, 'dim': 1200, 'hidden_size': 256, 'lr': 0.01, 'sdim': 200}
    for i in range(37):
        ...
        model.train(batch_size=int(10), epoch=int(100))
        # model.load_model()
        # model.get_aucs(100)
        # model.test(100)
